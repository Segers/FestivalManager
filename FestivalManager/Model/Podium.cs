...
namespace Project_MVVM.Model
{
    class Podium
    {
        ...
        public static Podium GetStageByID(int id)
        {
            Podium gevondenStage = new Podium();
            DbParameter par = Database.AddParameter("@ID", id);
            DbDataReader reader = Database.GetData("SELECT * FROM Stage WHERE StageID=@ID", par);
            while (reader.Read())
            {
                gevondenStage = MaakPodiumAan(reader);
